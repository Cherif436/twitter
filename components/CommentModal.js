      ...
      {open && (
        <Modal isOpen={open} onRequestClose={() => setOpen(false)} className="max-w-lg w-[90%] absolute top-24 left-[50%] translate-x-[-50%] bg-white border-2 border-gray-200 rounded-xl shadow-md">
          <div className="p-1">
            ...
            <p className="text-gray-500 text-[15px] sm:text-[16px] ml-16 mb-2">{post?.data()?.text.slice(0,120)+"..."}</p>

              <div className="flex p-3 space-x-3">
                <img src={session.user.image} alt="user-img" className="rounded-full cursor-pointer w-11 h-11 hover:brightness-95" />
                <div className="w-full divide-y divide-gray-200">
                  <div className="">
                    <textarea className="w-full border-none focus:ring-0 text-lg placeholder-gray-700 tracking-wide min-h-[50px] text-gray-700" rows="2" placeholder="Tweet you reply" value={input} onChange={(e) => setInput(e.target.value)}></textarea>
                  </div>
